...
export type EpisodeFields = { number: number; name: string; synopsis: string; duration_seconds: number | null };
...
export function EpisodeRow({ episode, disabled, actions }: { episode: EpisodeResponse; disabled: boolean; actions: EpisodeActions }) {
  ...
  useEffect(() => {
    setName(episode.name); setNumber(String(episode.number)); setSynopsis(episode.synopsis);
  ...
  return <form className="series-episode" aria-label={`第 ${episode.number} 集 · ${episode.name}`} onSubmit={(event) => {
    ...
    void actions.save(episode, { number: Number(number), name, synopsis, duration_seconds: minutes === "" ? null : Math.round(Number(minutes) * 60) }, file, publish, () => setFile(null));
  }}>
    ...
    <Field label="单集简介" className="movie-field-wide"><textarea rows={2} disabled={disabled || readOnly} value={synopsis} onChange={(e) => setSynopsis(e.target.value)} /></Field>
